...
async function textmakerCommand(sock, chatId, message, q, type) {
    try {
        if (!q) {
            return await sock.sendMessage(chatId, messageTemplates.error("⛧ Offer text for the shadows to inscribe\nExample: .metallic Nick"));
        }
        ...
        try {
            ...
            switch (type) {
                ...
                case 'fire':
                    ...
                    break;
                default:
                    return await sock.sendMessage(chatId, messageTemplates.error("☠️ Unknown rune (invalid text generator type)"));
            }
            ...
            await sock.sendMessage(chatId, messageTemplates.error(`☠️ Error: ${error.message}`));
        }
    } catch (error) {
        console.error('Error in textmaker command:', error);
        await sock.sendMessage(chatId, messageTemplates.error("☠️ The shadows faltered. Please try again later."));
    }
}
