    ...
    try {
        ...
        if (!text) {
            return await sock.sendMessage(chatId, { 
                text: "⛧ Offer an Instagram link and the shadows will claim it."
            });
        }
        ...
        if (!isValidUrl) {
            return await sock.sendMessage(chatId, { 
                text: "☠️ That is not a valid Instagram link. Bring a proper post, reel, or video link."
            });
        }
        ...
        if (!downloadData || !downloadData.data || downloadData.data.length === 0) {
            return await sock.sendMessage(chatId, { 
                text: "☠️ No media found at that link. The post may be sealed away (private) or the link is broken."
            });
        }
        ...
        if (mediaToDownload.length === 0) {
            return await sock.sendMessage(chatId, { 
                text: "☠️ No valid media could be extracted. The post may be private or the trail went cold."
            });
        }
        ...
        await sock.sendMessage(chatId, { 
            text: "☠️ The shadows faltered while extracting that link. Try again."
        });
    }
}
