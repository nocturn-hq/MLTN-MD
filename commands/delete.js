...
async function deleteCommand(sock, chatId, message, senderId) {
    try {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: '👑 Grant the Monarch admin power to erase these messages.' }, { quoted: message });
            return;
        }

        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: '👑 Only the Monarch\'s Chosen (Admins) can use .delete.' }, { quoted: message });
            return;
        }
        ...
        // If no number provided and not replying/mentioning, show usage message
            ...
            await sock.sendMessage(chatId, { 
                text: '👑 Name your target for erasure.\n\nUsage:\n• `.del 5` - Erase last 5 messages from the group\n• `.del 3 @user` - Erase last 3 messages from @user\n• `.del 2` (reply to message) - Erase last 2 messages from replied user' 
            }, { quoted: message });
            return;
        }
        ...
        if (toDelete.length === 0) {
            const errorMsg = deleteGroupMessages 
                ? '🌑 No recent messages found in the group to erase.' 
        ...
        await sock.sendMessage(chatId, { text: '💀 The shadows failed to erase these messages.' }, { quoted: message });
    }
}
