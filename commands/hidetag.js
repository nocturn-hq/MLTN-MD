...
async function hideTagCommand(sock, chatId, senderId, messageText, replyMessage, message) {
    ...
    if (!isBotAdmin) {
        await sock.sendMessage(chatId, { text: '⛧ Grant the bot admin power first, or it cannot bend this domain.' }, { quoted: message });
        return;
    }

    if (!isSenderAdmin) {
        await sock.sendMessage(chatId, { text: '⛧ Only Elite Commanders may summon the shadows with .hidetag.' }, { quoted: message });
        return;
    }
    ...
    if (replyMessage) {
        ...
        await sock.sendMessage(chatId, { text: messageText || '⛧ Summoned from the shadows (admins spared).', mentions: nonAdmins });
    }
}
