...
// Dynamically scope the local folder per bot instance to prevent collisions
const SESSION_DIR = path.join(process.cwd(), `session_${BOT_ID}`);
let client = null;
...
async function getCollection() {
    ...
    collectionPromise = (async () => {
        ...
        console.log(`👑 Bound to the Shadow Archive (MongoDB connected for botId="${BOT_ID}").`);
        return client.db(DB_NAME).collection(COLLECTION_NAME);
    })();
    return collectionPromise;
}
// Pull every saved session file down from MongoDB into ./session_[BOT_ID].
...
// Push every file currently in ./session_[BOT_ID] up into MongoDB.
...
module.exports = { restoreSession, backupSession, clearSession, SESSION_DIR };
